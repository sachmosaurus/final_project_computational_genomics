...
# ---------------------------------------------------------------------------
# 3.  CIGAR parser + classifier  (identical logic to pipeline.py)
# ---------------------------------------------------------------------------
...
def classify_cigar(cigar: str) -> str:
    """
    Classify a SAM CIGAR into {EXACT, SUB, INDEL, COMPLEX}.

    SAM uses M for both match and mismatch; we can't distinguish them from
    the CIGAR alone.  Instead:
      - EXACT  : only M ops, and we'll assume EXACT only when length == read length
                 and no I/D (the Biopython classifier does this too)
      - SUB    : only M ops (mismatches possible, no gaps)
      - INDEL  : has I or D (or both), no mismatches detectable from CIGAR alone
      - COMPLEX: has both M (with likely mismatches) and I/D

    Note: SAM M is ambiguous — it covers both matches and mismatches.
    We infer SUB when there are only M ops but the read isn't an exact copy
    of the reference.  Since we can't detect mismatches from CIGAR alone in
    SAM (unlike Biopython which has the explicit alignment), we use a
    proxy: if only M ops → could be EXACT or SUB; if also I/D → INDEL or COMPLEX.

    For our purposes the critical distinction is INDEL (has gaps) vs SUB (no gaps).
    """
    if cigar in ("*", ""):
        return "UNMAPPED"
    ops = parse_cigar(cigar)
    op_types = {op for _, op in ops}
    # Soft/hard clips count as local trimming — flag separately
    has_clip = bool(op_types & {"S", "H"})
    has_gap  = bool(op_types & {"I", "D"})
    # M is the only non-gap, non-clip op we expect here
    has_m    = "M" in op_types

    if not has_m and not has_gap:
        return "UNMAPPED"  # shouldn't happen on mapped reads
    if has_gap and has_m:
        return "COMPLEX"
    if has_gap and not has_m:
        return "INDEL"
    # Only M ops — SUB (or EXACT, but we can't tell without the sequence diff)
    # We'll call it SUB to match pipeline.py's convention for non-exact reads.
    # The corpus has no true exact matches (every case has exactly one mutation),
    # so this is safe.
    return "SUB"

def cigar_category(cigar: str, has_soft_clip_only: bool = False) -> str:
    """Wrapper that also handles fully soft-clipped (EXACT-looking but local) reads."""
    if cigar in ("*", ""):
        return "UNMAPPED"
    ops = parse_cigar(cigar)
    op_types = {op for _, op in ops}
    ...
    if has_gap:
        return "INDEL"
    if has_clip:
        # M + soft-clip only: local alignment trimmed the end → EXACT from
        # the aligner's perspective (the clipped bases were mismatches it dropped)
        return "EXACT"
    return "SUB"
    ...
    with open(sam_path) as fh:
        for line in fh:
            ...
            if flag & 0x100 or flag & 0x800:
                continue
            # Skip if already have a primary for this read (shouldn't happen with -c 1)
            md = ""
